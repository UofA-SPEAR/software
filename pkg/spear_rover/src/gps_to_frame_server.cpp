...
bool gps_to_frame(spear_rover::GpsToFrame::Request& req,
  ...
  RobotLocalization::NavsatConversions::LLtoUTM(lat, lon, utm_y, utm_x,
                                                utm_zone);

  const tf::Stamped<tf::Point> utm_point({utm_x, utm_y, alt}, ros::Time::now(),
                                         "utm");
  ...
  while (true) {
    try {
      ...
      auto message = std::string("Could not transform utm -> ") +
                     req.frame.data + std::string(": ") + err.what();
