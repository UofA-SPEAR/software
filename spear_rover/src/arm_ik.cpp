...
// Placeholder values, although actual values shouldn't matter as long as the ratios are maintained (currently they are
// not)
const float BICEP_LENGTH = 1.0;
const float FOREARM_LENGTH = 1.0;
const float WRIST_LENGTH = 1.0;

// float SHOULDER_OFFSET = 1.0;  Should offset doesn't matter; move the coordinate system up by this amount and the
...
struct angles toAnglesWithOrientation(int x, int y, int z, float alpha)
{
  ...
  // Needs C++ 11
  angles = toAnglesInPlane(hypot(xNorm, yNorm) - WRIST_LENGTH * cos(alpha), zNorm - WRIST_LENGTH * sin(alpha));
...
struct angles toAnglesInPlane(float x, float y)
{
  ...
  angles.elbowPitch =
      acos((pow(BICEP_LENGTH, 2) + pow(FOREARM_LENGTH, 2) - xyReachSquared) / (2 * BICEP_LENGTH * FOREARM_LENGTH));

  angles.shoulderPitch = acos((pow(BICEP_LENGTH, 2) + xyReachSquared - pow(FOREARM_LENGTH, 2)) /
